...
async function main() {
  ...
  // android/app/build.gradle is generated by `expo prebuild` and ignored in
  // this repo (CNG layout) — only commit what git actually tracks.
  const gradleIgnored =
    sh('git check-ignore -q android/app/build.gradle && echo yes || echo no') === 'yes';

  // A dirty tree is fine when the dirt is only OUR release files (app.json /
  // android/**) from an earlier aborted attempt — anything else is an error.
  const dirtyFiles = sh('git status --porcelain')
    .split('\n')
    .filter(Boolean)
    .map((l) => l.slice(3).trim().replace(/^"|"$/g, ''));
  const blocking = dirtyFiles.filter(
    (f) => f !== 'app.json' && !f.startsWith('android/')
  );
  if (blocking.length > 0 && !noCommit) {
    console.error(
      `✗ Working tree has unrelated changes:\n   ${blocking.join('\n   ')}\n  Commit or stash them first (or pass --no-commit).`
    );
    process.exit(1);
  }
  ...
  if (!noCommit) {
    ...
    if (!gradleIgnored) {
      ...
      // CNG layout: gradle values are regenerated from app.json by
      // `expo prebuild`, so the local patch is enough for this build.
      console.log('⏭  android/ is gitignored — version lives in app.json only');
    }
