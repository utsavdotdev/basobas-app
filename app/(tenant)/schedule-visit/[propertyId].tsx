  ...
  return (
    <SafeAreaView edges={['top']} className="flex-1 bg-bg">
      ...
      <ScrollView className="px-6" contentContainerStyle={{ paddingTop: 16, paddingBottom: 100 }}>
        ...
        {/* Date strip */}
        <Text className="mb-3 font-semibold text-h3 text-ink">Select Date</Text>
        ...
        {/* Time grid */}
        <Text className="mb-3 font-semibold text-h3 text-ink">Select Time</Text>
        <View className="mb-5 flex-row flex-wrap">
          {['9:00 AM', '10:00 AM', '11:00 AM', '1:00 PM', '2:00 PM', '3:00 PM'].map((time, i) => (
            <Pressable
              key={time}
              className={`mb-2 mr-2 rounded-lg px-4 py-3 ${i === 1 ? 'bg-ink' : 'bg-canvas'}`}>
              <Text className={`font-medium text-body-sm ${i === 1 ? 'text-white' : 'text-ink'}`}>
                {time}
              </Text>
            </Pressable>
          ))}
        ...
        {/* Visit type */}
        <Text className="mb-3 font-semibold text-h3 text-ink">Visit Type</Text>
        <View className="mb-5 flex-row">
          <Pressable className="mr-2 flex-1 items-center rounded-lg bg-ink py-3">
