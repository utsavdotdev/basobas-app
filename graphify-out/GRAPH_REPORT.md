# Graph Report - basobas-app  (2026-07-05)

## Corpus Check
- 133 files · ~47,096 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 718 nodes · 1000 edges · 72 communities (43 shown, 29 thin omitted)
...
9. `Project Documentation Index` - 11 edges
10. `getErrorMessage()` - 10 edges
...
Cohesion: 0.06
Nodes (50): ConfirmationScreen(), DetailRowProps, styles, SummaryCardProps, DocTypeChipProps, DocumentUploadZoneProps, KYCLandlordScreen(), styles (+42 more)
...
Cohesion: 0.09
Nodes (21): styles, FeatureSlide, s, SLIDES, Onboarding Steps, Verification Builds Trust, MapIllustration(), styles (+13 more)
...
Cohesion: 0.16
Nodes (12): maskPhone(), OTPVerificationScreen(), s, countryCodeToFlag(), PhoneEntryScreen(), PREFERRED, styles, OTPCell (+4 more)
...
Cohesion: 0.09
Nodes (25): Index(), RootLayout(), RootLayout (auth/role routing), TenantLayout, DevMode, getDevMode(), getInitialHref(), clerkTokenCache (+17 more)
...
## Knowledge Gaps
- **330 isolated node(s):** `name`, `slug`, `version`, `scheme`, `favicon` (+325 more)
...
- **What is the exact relationship between `HomeTab` and `MapIllustration`?**
  _Edge tagged AMBIGUOUS (relation: conceptually_related_to) - confidence is low._
- **Why does `useOnboardingStore` connect `Confirmation Screen & Detail Rows` to `Onboarding Slides & Step Content`, `Product Docs: Vision & Experience`?**
  _High betweenness centrality (0.083) - this node is a cross-community bridge._
- **Are the 3 inferred relationships involving `useOnboardingStore` (e.g. with `KYC Mandatory For Landlord` and `Onboarding Steps`) actually correct?**
...
- **Should `Phone Auth & Public Landlord Profile` be split into smaller, more focused modules?**
